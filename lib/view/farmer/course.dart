...
class CoursePage extends HookConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    Future<void> init() async {
      ...
      dynamic learnddata = await userControllerW.getAllLearn(context);
      // filter all data by type FOOD HEALTH MEDICINE add in respective list
      if (learnddata != null) {
        for (var i = 0; i < learnddata.length; i++) {
          if (learnddata[i]["type"] == "FOOD") {
            foodCourse.value = [...foodCourse.value, learnddata[i]];
          } else if (learnddata[i]["type"] == "HEALTH") {
            healthCourse.value = [...healthCourse.value, learnddata[i]];
          } else if (learnddata[i]["type"] == "MEDICINE") {
            medicineCourse.value = [...medicineCourse.value, learnddata[i]];
          }
        }
      }
                ...
                children: [
                  ...
                  SingleChildScrollView(
                    ...
                    child: Row(
                      ...
                      mainAxisAlignment: MainAxisAlignment.start,
                      children: [
                        for (var i = 0; i < healthCourse.value.length; i++) ...[
                          CowCard(
